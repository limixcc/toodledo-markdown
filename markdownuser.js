// ==UserScript==
// @name        toodledo-markdown
// @namespace   limix.toodledo
// @include     http://www.toodledo.com/*
// @version     1
// @grant       none
// @copyright  2014/09, limix
// ==/UserScript==
...
block: {
  ...
  blockquote: function blockquote(block, next) {
    ...
    old_tree = this.tree,
    processedBlock = this.toTree(input, [
      'blockquote'
    ]),
...
// added by limix for parse note to html
function getNotes() {
...
function limixAspect(){
  ...
  var h1 = as.before(window, 'doSearch', function (a) {
    var inputText = window.$F("searchField");
    var array = new Array();
    array = inputText.split(" ");
    if (array.shift() == "detail"){
      console.log("IIIII: LIMIX_TODO do detail");
      document.getElementById("searchField").value = array.join(" ");
      return "limix_break";
    }
  ...
  function aspect(type) {
    return function (target, methodName, advice) {
      ...
      if (!exist || exist.target != target) {
        dispatcher = target[methodName] = function (a) {
          // before methods
          var beforeArr = dispatcher.before;
          var args = arguments;
          for (var l = beforeArr.length; l--; ) {
            var result = beforeArr[l].advice.apply(this, args);
            if("limix_break" == result){
              console.log("IIIII: the result is " + result);
              return;
            }
            args = result || args;
          }
